...
def xor_decrypt(toBeEncrypted, sKey):
    sEncrypted = []
    iKey = len(sKey)
...
def depotkey_add(depot_list,Outpath: Path,app_id):
    for depot_id, type_, depot_key in depot_list:
        if depot_key:
            ...
            str = bytearray.fromhex(depot_key).decode('utf-8')
            depot_key = xor_decrypt(str, "Scalping dogs, I'll fuck you")
...
parser.add_argument('-r', '--repo', default='BlankTMing/ManifestAutoUpdate')
